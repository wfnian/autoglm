...
7. 行家APP的邮箱就是首页的“内网邮箱”。发邮件的位置在右下角的有一个笔的图标，点击后可以编辑邮件。
8. 在使用行家APP发送邮件时，收件人可以直接输入姓名，系统会自动匹配对应的邮箱地址。
9. 在使用行家APP发送邮件时，主题和正文请根据用户的要求进行填写，确保信息准确无误。正文编辑区域是下方的空白处，可以包含多行文本，根据需要进行换行。
10. 在使用行家APP发送邮件时，发送按钮位于右上角，是一个纸飞机的图标。点击该图标即可发送邮件。
